...
After choosing the best model and training it on the relevant data, I decided to do some inferential data vizualization. My goal here was to deduce which features were the most predictive and, thus, create an additional layer of interpretation for the end-user. However, before proceeding, it's important that I mention how feature importance was determined. As people may or may not know, sci-kit learn's feature importance method works by selecting the features that most reduce uncertainty (for classifiers) or variance (for regressors - as in this case). However, the problem with just choosing feature that, on average, reduct the most uncertainty is that they tend to perform sub-optimally in situations where features vary in their meaursement scales or number of categories. Therefore, I chose to employ **permutation importance** instead, as recent research has shown it to be much more reliable when infering importance from a model. More specifically, permutation importance works by randomly shuffling the values in a predictor variable and assessing the subsequent change in model performance. My results can be seen in the image below.
